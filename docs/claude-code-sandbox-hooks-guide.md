...
問題：Codex CLI 在 Claude Code sandbox 中持續崩潰

症狀
- codex-agent start 啟動後 5-6 秒內立即結束
- files_modified: null，無任何實際工作產出
- codex-agent capture 輸出顯示 Rust panic：

thread 'main' panicked at system-configuration-0.6.1/src/dynamic_store.rs:154:1:
Attempted to create a NULL object.

根本原因
- Claude Code 的 Bash 工具預設在 sandbox 中執行，限制了檔案系統與網路存取
- Codex CLI（Rust 編譯的二進位）啟動時會透過 system-configuration crate 讀取 macOS 的 DNS/網路組態（SCDynamicStore）
- sandbox 阻擋了對 macOS System Configuration framework 的存取，導致 SCDynamicStoreCreate 回傳 NULL
- Rust crate 對 NULL 做了 unwrap() → panic → 整個 codex 程序直接崩潰
- 這發生在 Codex 能做任何工作之前，所以 codex-agent health 顯示 "Ready" 但實際執行時必定失敗

診斷流程
1. codex-agent health → 回報 OK（health check 不觸發 dynamic store）
2. codex-agent start ... → 6 秒完成，files_modified: null
3. codex-agent capture <id> → 顯示 Rust panic
4. 直接跑 codex exec "simple task" → 同樣 panic（確認非 codex-agent wrapper 的問題）
5. 加 dangerouslyDisableSandbox: true 後 codex exec → 正常執行

解決方案
所有 codex-agent 相關的 Bash 呼叫都必須加上 dangerouslyDisableSandbox: true，包括：
- codex-agent start ...（啟動 agent）
- codex-agent await-turn ...（等待完成）
- codex-agent capture/output/status/jobs（讀取結果）

影響範圍
- 平台：macOS（Darwin）限定，Linux 不受影響（無 SCDynamicStore）
- Codex CLI 版本：v0.115.0（system-configuration crate v0.6.1）
- Claude Code sandbox 的網路/系統 API 限制是觸發條件

建議
- Codex Orchestrator 文件應註明 macOS 使用者需要 sandbox bypass
- 或在 codex-agent wrapper 的啟動腳本中偵測 sandbox 環境並提示
- 長期修復：向 OpenAI Codex CLI 回報，對 SCDynamicStoreCreate 的 NULL 回傳做 graceful fallback 而非 panic

---
等主 agent 完成當前批次後，請直接跟它說「把 sandbox 問題寫入 codex-orchestrator docs」，它就能用上面的內容寫入檔案。