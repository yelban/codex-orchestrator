...
| `/codex-plan` | **Codex 5.5 high** | 深讀程式碼後，從 coding 視角產出實作計畫 |
| `/codex-orchestrator` | **Codex 5.5 high** + **Gemini 3.1 Pro**（複數 agent）| 平行執行：實作、測試、review、跨模型分析 |
...
/codex-plan              → Claude Opus 問需求 → 找相關檔案
                           → Codex 5.5 high 深讀 → codex-plan.md
      ↓
...
|--|--|--|
| **規劃者** | Opus 4.6（推理強，但 coding 不是專長） | Claude Opus 問需求 + **Codex 5.5 high 深讀程式碼後規劃**（規劃者和執行者同模型） |
| **執行者** | Opus 4.6 循序實作 | **Codex 5.5**（複數 agent，可平行） |
...
| **計畫品質** | Opus 推理計畫（通用型） | Codex 5.5 讀完實際程式碼後的計畫（貼近程式碼） |
...
- Opus 4.6 不是專門為 coding 優化，和 Codex 5.5 相比實作深度有差距
...
- Codex 5.5 high 規劃：讀過真正的程式碼後才出計畫，比純推理更貼近實際
...
| 複雜功能、需要深入規劃、架構影響大 | **codex-plan → codex-orchestrator**（Codex 5.5 規劃+執行）|
...
>
> **三 Skill 組合**：適合中大型任務，Codex 5.5 規劃+執行比 Opus 4.6 直接做更深，平行能力讓時間成本大幅壓縮。
>
> 兩者不互斥 — Opus 4.6 是 orchestrator，Codex 5.5 是執行軍團。
