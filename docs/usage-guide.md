...
| 設定 | 必要性 | 理由 |
|------|--------|------|
| `network.allowAllUnixSockets: true` | **強制** | Codex CLI (Rust) 透過 Unix socket 連接 macOS `configd` 讀取 DNS 設定。為 `false` 時 codex 直接 panic |
| `enableWeakerNetworkIsolation: true` | **推薦** | 允許 Go 工具（`gh`、`gcloud`、`terraform`）驗證 TLS 憑證。未設定時 `gh release create` 等指令會 TLS 錯誤 |
| `excludedCommands` | 推薦 | 避免每次執行 codex-agent 都需確認權限 |

修改後需**重啟 Claude Code session**。

> **Fork repo 使用者注意**：如果 `origin` 指向 fork，需在終端（非 Claude Code 內）執行一次
> `gh repo set-default <owner/repo>`，否則 `gh` 指令不知道操作哪個 repo。
...
| `/codex-plan` | **Codex 5.4 xhigh** | 深讀程式碼後，從 coding 視角產出實作計畫 |
| `/codex-orchestrator` | **Codex 5.4 xhigh** + **Gemini 3.1 Pro**（複數 agent）| 平行執行：實作、測試、review、跨模型分析 |
...
/codex-plan              → Claude Opus 問需求 → 找相關檔案
                           → Codex 5.4 xhigh 深讀 → codex-plan.md
      ↓
...
|--|--|--|
| **規劃者** | Opus 4.6（推理強，但 coding 不是專長） | Claude Opus 問需求 + **Codex 5.4 xhigh 深讀程式碼後規劃**（規劃者和執行者同模型） |
| **執行者** | Opus 4.6 循序實作 | **Codex 5.4**（複數 agent，可平行） |
...
| **計畫品質** | Opus 推理計畫（通用型） | Codex 5.4 讀完實際程式碼後的計畫（貼近程式碼） |
...
- Opus 4.6 不是專門為 coding 優化，和 Codex 5.4 相比實作深度有差距
...
- Codex 5.4 xhigh 規劃：讀過真正的程式碼後才出計畫，比純推理更貼近實際
...
| 複雜功能、需要深入規劃、架構影響大 | **codex-plan → codex-orchestrator**（Codex 5.4 規劃+執行）|
...
>
> **三 Skill 組合**：適合中大型任務，Codex 5.4 規劃+執行比 Opus 4.6 直接做更深，平行能力讓時間成本大幅壓縮。
>
> 兩者不互斥 — Opus 4.6 是 orchestrator，Codex 5.4 是執行軍團。

---

## Plugin 更新流程

當 `yelban/codex-orchestrator` 有新版本推送後，依以下步驟更新：

### 標準更新流程

```bash
# Step 1：重新安裝（拉取最新 marketplace 資料）
/plugin install codex-orchestrator

# Step 2：在 /plugin 選單中選擇 codex-orchestrator → update
/plugin
# → 選擇 codex-orchestrator → update
...
三個步驟缺一不可：
- `install` 更新 marketplace clone 和 plugin cache
- `update` 在選單中觸發版本更新
- `reload-plugins` 讓新版 SKILL.md 生效

### 驗證更新成功

```bash
/plugin
# 應顯示 codex-orchestrator 版本為最新版（如 1.1.0）
```

### 如果 `/plugin install` 顯示 "already installed"

這表示 marketplace clone 尚未同步。先在**終端**（非 Claude Code 內）手動 pull：
...
> **注意**：Claude Code 內執行 git pull 可能被 sandbox 擋住（`Operation not permitted`），
> 因此需在一般終端操作。
...
| 位置 | 說明 | 更新方式 |
|------|------|----------|
| `~/zoo/codex-orchestrator/` | 原始碼 | `git push` |
| `~/.claude/plugins/marketplaces/codex-orchestrator-marketplace/` | Marketplace clone | `/plugin install` 或手動 `git pull` |
| `~/.claude/plugins/cache/codex-orchestrator-marketplace/` | Plugin cache（Claude Code 實際讀取） | `/plugin` → update |

Claude Code 啟動時讀取 **plugin cache** 的 SKILL.md，所以三個位置都要同步才能生效。
